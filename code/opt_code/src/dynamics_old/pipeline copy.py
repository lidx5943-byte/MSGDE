...
def _evolve_single_scale(args) -> Tuple[int, np.ndarray, Dict[str, Any], bool]:
    ...
    try:
        ...
        solver_mode = config_dict.get('solver_mode', 'legacy')  # 默认使用legacy模式
        
        # 根据求解器模式选择实现方式
        if solver_mode == 'optimized':
            # 使用优化版本（固定步长迭代，更快约1.5-3%）
            ...
            # 使用Legacy版本（与老版完全一致，使用t_eval逐步推进）
            # 时间点（与老版一致）
            t_span = (0, n_steps * h)
...
class DynamicsAnalysisPipeline:
    ...
    def _get_config_dict(self) -> Dict[str, Any]:
        ...
        if self.config is not None:
            ...
            return {
                ...
                'solver_mode': getattr(dynamics.numerical, 'solver_mode', 'legacy'),  # 求解器模式
                # 分析参数
            ...
            return {
                ...
                'solver_mode': 'legacy',  # 默认使用legacy模式以保持与老版一致
                'n_scales': 5,
                'time_steps': 11000,
    ...
    def _print_config(self, config_dict: Dict, n_scales: int, n_jobs: int):
        ...
        rows = [
            ...
            ["求解器模式", config_dict.get('solver_mode', 'legacy')],
