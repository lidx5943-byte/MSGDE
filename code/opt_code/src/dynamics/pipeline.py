...
def _evolve_single_scale(args) -> Tuple[int, np.ndarray, Dict[str, Any], bool]:
    ...
    ----
    args : tuple
        (scale_idx, L, config_dict, progress_dict)
        如果 progress_dict 为 None，则不报告进度
    ...
    """
    if len(args) == 4:
        scale_idx, L, config_dict, progress_dict = args
    else:
    ...
    try:
        ...
        h = config_dict['time_step']
        epsilon = config_dict['coupling_strength']
        coupling_mode = config_dict['coupling_mode']
        use_periodic = config_dict['use_periodic_boundary']
        
        # 初始化进度（如果提供了进度字典）
        if progress_dict is not None:
            progress_dict[scale_idx] = {'current': 0, 'total': n_steps, 'status': 'running'}
        
        # 时间点（与老版一致）
        t_span = (0, n_steps * h)
        t_eval = np.linspace(0, n_steps * h, n_steps + 1)
        
        # 使用与老版完全一致的求解方式
        # 如果需要报告进度，使用带进度报告的求解函数
        if progress_dict is not None:
            # 使用带进度报告的求解
            sol = _solve_with_progress(
                L, oscillator, config_dict, initial_state.flatten(),
                t_span, t_eval, h, progress_dict, scale_idx
            )
        else:
            # 使用标准求解（无进度报告）
            if config_dict['method'].lower() == 'euler':
                ...
                class TempConfig:
                    def __init__(self, config_dict):
                        self.coupling_mode = config_dict['coupling_mode']
                        self.use_periodic_boundary = config_dict['use_periodic_boundary']
                        self.rk = config_dict['rk']
                        self.epsilon = config_dict['coupling_strength']
                
                temp_config = TempConfig(config_dict)
                
                # 导入老版风格的求解函数
                from .solvers import _euler_solve_legacy
                sol = _euler_solve_legacy(
                    L, oscillator, epsilon, t_span, 
                    initial_state.flatten(), t_eval, h, temp_config,
                    state_clip_min=config_dict['state_clip_min'],
                    state_clip_max=config_dict['state_clip_max'],
                    dynamics_clip_min=config_dict['dynamics_clip_min'],
                    dynamics_clip_max=config_dict['dynamics_clip_max']
                ...
                # RK4方法
                from .solvers import _rk4_solve_legacy
                sol = _rk4_solve_legacy(
                    L, oscillator, epsilon, t_span,
                    initial_state.flatten(), t_eval, h, coupling_mode,
                    state_clip_min=config_dict['state_clip_min'],
                    state_clip_max=config_dict['state_clip_max'],
                    dynamics_clip_min=config_dict['dynamics_clip_min'],
                    dynamics_clip_max=config_dict['dynamics_clip_max']
        ...
        # 重塑轨迹数据（与老版一致）
        n_nodes = L.shape[0]
        trajectories = sol['y'].T.reshape(n_nodes, n_steps + 1, 3)
        ...
        # 更新进度为完成
        if progress_dict is not None:
            progress_dict[scale_idx] = {'current': n_steps, 'total': n_steps, 'status': 'completed'}
        ...
        # 更新进度为失败
        if progress_dict is not None:
            progress_dict[scale_idx] = {'current': 0, 'total': config_dict['time_steps'], 'status': 'failed'}
        
        return (scale_idx, trajectories, stats, False)


def _solve_with_progress(L, oscillator, config_dict, y0, t_span, t_eval, h, 
                         progress_dict, scale_idx):
    """
    带进度报告的求解函数
    """
    from .solvers import BaseSolver, SolverConfig, _rk4_step_legacy, _euler_step_with_periodic_legacy
    ...
    while eval_idx < n_eval:
        ...
        if config_dict['method'].lower() == 'euler':
            if use_periodic:
                y_next = _euler_step_with_periodic_legacy(
                    t_current, y_current, dt, L, oscillator, epsilon, temp_config
                )
            else:
        ...
        # 更新进度
        current_step = int(t_current / h)
        if current_step - last_progress_update >= progress_interval:
            if progress_dict is not None:
                progress_dict[scale_idx] = {
                    'current': min(current_step, n_steps),
                    'total': n_steps,
                    'status': 'running'
                }
...
class DynamicsAnalysisPipeline:
    ...
    def _run_evolution_parallel(
        ...
        # 准备任务（包含进度字典）
        tasks = [
            (scale_idx, laplacian_family[scale_idx], config_dict, progress_dict)
            for scale_idx in range(n_scales)
        ]
        ...
        with Progress(
            ...
            for scale_idx in range(n_scales):
                task_id = progress.add_task(
                    ...
                    total=config_dict['time_steps']
                )
                scale_tasks[scale_idx] = task_id
            ...
            def update_progress():
                ...
                while not stop_progress_thread.is_set():
                    for scale_idx in range(n_scales):
                        if scale_idx in progress_dict:
                            prog_info = progress_dict[scale_idx]
                            current = prog_info.get('current', 0)
                            total = prog_info.get('total', config_dict['time_steps'])
                            status = prog_info.get('status', 'running')
                            
                            if scale_idx in scale_tasks:
                                progress.update(
                                    scale_tasks[scale_idx],
                                    completed=current,
                                    total=total
                                )
                                ...
                                if status == 'completed':
                                    progress.update(
                                        scale_tasks[scale_idx],
                                        description=f"[green]✓ 尺度 {scale_idx + 1}/{n_scales}"
                                    )
                                elif status == 'failed':
                                    progress.update(
                                        scale_tasks[scale_idx],
                                        description=f"[red]✗ 尺度 {scale_idx + 1}/{n_scales}"
                                    )
                ...
                with ProcessPoolExecutor(max_workers=n_jobs) as executor:
                    ...
                    for future in as_completed(futures):
                        ...
                        try:
                            ...
                            # 移除该尺度的进度任务
                            if idx in scale_tasks:
                                progress.remove_task(scale_tasks[idx])
                            ...
                            # 移除该尺度的进度任务
                            if scale_idx in scale_tasks:
                                progress.remove_task(scale_tasks[scale_idx])
            finally:
                # 停止进度更新线程
