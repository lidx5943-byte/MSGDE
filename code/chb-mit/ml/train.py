...
def preprocess_data(data_path: str, label_path: str, scale_k: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    参数:
        data_path: 输入数据路径，形状为 (10, 500, 1001, 3)
        label_path: 标签数据路径，形状为 (500,)
        scale_k: 选择的尺度K (0-9)
    ...
    """
    # 加载数据
    data = np.load(data_path)  # 形状: (10, 500, 1001, 3)
    labels = np.load(label_path)  # 形状: (500,)
    
    # 提取指定尺度K的数据
    scale_data = data[scale_k, :, :, :]  # 形状: (500, 1001, 3)
    
    # 提取统计特征
    X = extract_statistical_features(scale_data)  # 形状: (500, 6)
